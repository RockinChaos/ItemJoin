...
public class GuardAPI {
    ...
    */
	public static void saveReturnItems(final Player player, final String region, final String type, final Inventory craftView, final PlayerInventory inventory, final int clearType) {
		boolean doReturn = ((ItemJoin.getCore().getConfig("config.yml").getString("Clear-Items.Options") != null) ? StringUtils.splitIgnoreCase(ItemJoin.getCore().getConfig("config.yml").getString("Clear-Items.Options").replace(" ", ""), "RETURN", ",") : false);
