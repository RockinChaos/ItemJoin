...
public class Updater {
                        ...
                        input.close();
                 	       ...
                 	       try {
                 	    	 ...
                 	    	 minValue = Double.parseDouble(this.version.replace("-SNAPSHOT", "").replace("-BETA", "").replace("-ALPHA", "").replace("-RELEASE", "").replace(".", ""));
                	    	   ...
                	    	   sender.sendMessage(Utils.stripLogColors(sender, Prefix + ChatColor.RED + "An error has occurred when checking the plugin version!"));
                  ...
                  if (checker.updateNeeded(sender))
                    {
                	  ...
                	  sender.sendMessage(Utils.stripLogColors(sender, Prefix + ChatColor.GREEN + "Attempting to update from " + ChatColor.YELLOW + "v" + ItemJoin.getInstance().getDescription().getVersion() +  ChatColor.GREEN +  " to the new "  + ChatColor.YELLOW +  "v" + checker.getVersion()));
    				try {
						...
						sender.sendMessage(Utils.stripLogColors(sender, Prefix + ChatColor.RED + "An error has occurred while trying to update the plugin ItemJoin."));
    ...
    public static Boolean checkUpdates(CommandSender manager) {
        ...
        try {
        	...
        	sender.sendMessage(Utils.stripLogColors(sender, Prefix + ChatColor.RED + "Please contact the plugin developer!"));
        	sender.sendMessage(Utils.stripLogColors(sender, Prefix + ChatColor.RED + "Error is " + e + " C13904"));
        	if (ServerHandler.hasDebuggingMode()) { e.printStackTrace(); }
        	return false;
        }
    }
