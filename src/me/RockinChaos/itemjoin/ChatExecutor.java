...
public class ChatExecutor implements CommandExecutor {
	...
	*/
	...
	public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
		if (Execute.DEFAULT.accept(sender, args, 0)) {
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin Help &afor the help menu.", "&eClick to View the Help Menu.", "/itemjoin help", ClickAction.RUN_COMMAND);
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin Help 2 &afor the next page.", "&eClick to View the Next Page.", "/itemjoin help 2", ClickAction.RUN_COMMAND);
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin Help 3 &afor the next page.", "&eClick to View the Next Page.", "/itemjoin help 3", ClickAction.RUN_COMMAND);
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin Help 4 &afor the next page.", "&eClick to View the Next Page.", "/itemjoin help 4", ClickAction.RUN_COMMAND);
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin Help 5 &afor the next page.", "&eClick to View the Next Page.", "/itemjoin help 5", ClickAction.RUN_COMMAND);
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin Help 6 &afor the next page.", "&eClick to View the Next Page.", "/itemjoin help 6", ClickAction.RUN_COMMAND);
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin Help 7 &afor the next page.", "&eClick to View the Next Page.", "/itemjoin help 7", ClickAction.RUN_COMMAND);
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin Help 8 &afor the next page.", "&eClick to View the Next Page.", "/itemjoin help 8", ClickAction.RUN_COMMAND);
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin Help 9 &afor the next page.", "&eClick to View the Next Page.", "/itemjoin help 9", ClickAction.RUN_COMMAND);
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin Help 10 &afor the next page.", "&eClick to View the Next Page.", "/itemjoin help 10", ClickAction.RUN_COMMAND);
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&a&l&m]------------------&a&l[&e ItemJoin &a&l]&a&l&m-----------------[");
			ItemJoin.getCore().getLang().dispatchMessage(sender, "");
			String[] placeHolders = ItemJoin.getCore().getLang().newString(); placeHolders[0] = ((Player) sender).getWorld().getName();
			...
			SchedulerUtils.runAsync(() -> {
				ItemJoin.getCore().getUpdater().checkUpdates(sender, false); 
			});
			...
			SchedulerUtils.runAsync(() -> {
				ItemJoin.getCore().getUpdater().forceUpdates(sender); 
			});
			...
			if (executor.equals(Execute.GET))            { ItemJoin.getCore().getLang().sendLangMessage("commands.get.usageSyntax", sender); } 
			else if (executor.equals(Execute.GETALL))    { ItemJoin.getCore().getLang().sendLangMessage("commands.get.usageSyntax", sender); } 
			else if (executor.equals(Execute.REMOVE))    { ItemJoin.getCore().getLang().sendLangMessage("commands.remove.usageSyntax", sender); } 
			else if (executor.equals(Execute.REMOVEALL)) { ItemJoin.getCore().getLang().sendLangMessage("commands.remove.usageSyntax", sender); }
		} else if (!this.matchExecutor(args).hasSyntax(args, 0)) {
			Execute executor = this.matchExecutor(args);
			if (executor.equals(Execute.GET))               { ItemJoin.getCore().getLang().sendLangMessage("commands.get.badSyntax", sender); } 
			else if (executor.equals(Execute.GETONLINE))    { ItemJoin.getCore().getLang().sendLangMessage("commands.get.badOnlineSyntax", sender); } 
			else if (executor.equals(Execute.REMOVE))       { ItemJoin.getCore().getLang().sendLangMessage("commands.remove.badSyntax", sender); } 
			else if (executor.equals(Execute.REMOVEONLINE)) { ItemJoin.getCore().getLang().sendLangMessage("commands.remove.badOnlineSyntax", sender); }
			else if (executor.equals(Execute.PURGE))        { ItemJoin.getCore().getLang().sendLangMessage("commands.default.unknownCommand", sender); }
			else if (executor.equals(Execute.QUERY))        { ItemJoin.getCore().getLang().sendLangMessage("commands.query.badSyntax", sender); }
	...
	*/
	private void info(final CommandSender sender) {
		if (PlayerHandler.getHandItem((Player) sender) != null && PlayerHandler.getHandItem((Player) sender).getType() != Material.AIR) {
			...
			ItemJoin.getCore().getLang().dispatchMessage(sender, "&a&l&m]-----------------&a&l[&e Item Info &a&l]&a&l&m----------------[");
			ItemJoin.getCore().getLang().dispatchMessage(sender, " ");
		} else { ItemJoin.getCore().getLang().sendLangMessage("commands.item.noItemHeld", sender); }	
	}
	...
	*/
	private void list(final CommandSender sender, final int page) {
		...
		if (page != maxPage) { ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin List " + (page + 1) + " &afor the next page.", "&eClick to View the Next Page.", "/itemjoin list " + (page + 1), ClickAction.RUN_COMMAND); }
	...
	*/
	private void permissions(final CommandSender sender, final int page) {
		...
		if (page == 1) {
			...
			for (World world: Bukkit.getServer().getWorlds()) {
				...
				for (ItemMap item: ItemUtilities.getUtilities().getItems()) {
					if ((item.getPermissionNode() != null ? !customPermissions.contains(item.getPermissionNode()) : true) && item.inWorld(world) && ((probable != null && item.getConfigName().equals(probable.getConfigName())) || item.getProbability() == -1)) {
		...
		if (page != maxPage) { ItemJoin.getCore().getLang().dispatchMessage(sender, "&aType &a&l/ItemJoin Permissions " + (page + 1) + " &afor the next page.", "&eClick to View the Next Page.", "/itemjoin permissions " + (page + 1), ClickAction.RUN_COMMAND); }
	...
	*/
	...
	private void purge(final CommandSender sender, final String table, final String args) {
		String[] placeHolders = ItemJoin.getCore().getLang().newString(); placeHolders[1] = args; placeHolders[10] = table; 
		...
		if (!table.equalsIgnoreCase("Database")) { 
			...
			if (!table.equalsIgnoreCase("map-ids") && foundPlayer == null && !args.equalsIgnoreCase("ALL")) { ItemJoin.getCore().getLang().sendLangMessage("commands.default.noTarget", sender, placeHolders); return; } 
		...
		if (this.confirmationRequests.get(table + sender.getName()) != null && this.confirmationRequests.get(table + sender.getName()).equals(true)) {
			if (!table.equalsIgnoreCase("Database")) { 
				...
				if (dataObject != null) { ItemJoin.getCore().getSQL().removeData(dataObject); }
			} 
			else {
			ItemJoin.getCore().getData().setStarted(false);
				ItemJoin.getCore().getSQL().purgeDatabase(); {
					SchedulerUtils.runAsync(() -> {
						ItemJoin.getCore().getSQL().refresh(false); {
	...
	*/
	private void enable(final CommandSender sender, final String player, final String world, final int arguments) {
		...
		String[] placeHolders = ItemJoin.getCore().getLang().newString(); placeHolders[1] = (arguments >= 2 ? player : sender.getName()); placeHolders[0] = world;
		if (arguments >= 2 && argsPlayer == null) { ItemJoin.getCore().getLang().sendLangMessage("commands.default.noTarget", sender, placeHolders); return; }
		...
		} else { ItemJoin.getCore().getLang().sendLangMessage("commands.enabled." + (arguments == 3 ? "forPlayerWorldFailed" : (arguments == 2 ? "forPlayerFailed" : "globalPlayersFailed")), sender, placeHolders); }
	}
	...
	*/
	private void disable(final CommandSender sender, final String player, final String world, final int arguments) {
		...
		} else { ItemJoin.getCore().getLang().sendLangMessage("commands.disabled." + (arguments == 3 ? "forPlayerWorldFailed" : (arguments == 2 ? "forPlayerFailed" : "globalPlayersFailed")), sender, placeHolders); }
	}
	...
	*/
	private void handleItems(final CommandSender sender, final String[] args, final boolean remove) {
		...
		String[] placeHolders = ItemJoin.getCore().getLang().newString(); placeHolders[1] = (args.length >= 3 && argsPlayer != null ? argsPlayer.getName() : (args.length >= 3 ? args[2] : sender.getName())); placeHolders[3] = args[1];
		...
		for (ItemMap itemMap: ItemUtilities.getUtilities().getItems()) {
			if (itemMap.getConfigName().equalsIgnoreCase(args[1])) {
				...
				if ((remove && itemMap.hasItem(argsPlayer, true)) || (!remove && (itemMap.conditionMet(argsPlayer, "trigger-conditions", false) && (ItemUtilities.getUtilities().canOverwrite(argsPlayer, itemMap) && (amount != 0 || itemMap.isAlwaysGive() || !itemMap.hasItem(argsPlayer, false)))))) {
					if (remove || !PermissionsHandler.permissionEnabled("Permissions.Commands-Get") || (itemMap.hasPermission(argsPlayer, argsPlayer.getWorld()) && PermissionsHandler.permissionEnabled("Permissions.Commands-Get"))) {
						...
						if (!messageSent) { ItemJoin.getCore().getLang().sendLangMessage("commands." + (remove ? "remove.removedYou" : "get.givenYou"), argsPlayer, placeHolders); }
						if (!messageSent && (args.length >= 3 && !StringUtils.isInt(args[2]) && !sender.getName().equalsIgnoreCase(argsPlayer.getName()))) { placeHolders[1] = argsPlayer.getName(); ItemJoin.getCore().getLang().sendLangMessage("commands." + (remove ? "remove.removedTarget" : "get.givenTarget"), sender, placeHolders); }
	...
	*/
	private void handleOnline(final CommandSender sender, final String[] args, final boolean remove) {
		String[] placeHolders = ItemJoin.getCore().getLang().newString(); placeHolders[3] = args[1];
				...
				if (itemMap.getConfigName().equalsIgnoreCase(args[1])) {
					if (remove || !PermissionsHandler.permissionEnabled("Permissions.Commands-Get") || (itemMap.hasPermission(argsPlayer, argsPlayer.getWorld()) && PermissionsHandler.permissionEnabled("Permissions.Commands-Get"))) {
						if ((remove && itemMap.hasItem(argsPlayer, true)) || (!remove && (itemMap.conditionMet(argsPlayer, "trigger-conditions", false) && (ItemUtilities.getUtilities().canOverwrite(argsPlayer, itemMap) && (amount != 0 || itemMap.isAlwaysGive() || !itemMap.hasItem(argsPlayer, false)))))) {
							...
							if (!messageSent && !sender.getName().equalsIgnoreCase(argsPlayer.getName())) { ItemJoin.getCore().getLang().sendLangMessage("commands." + (remove ? "remove.removedYou" : "get.givenYou"), argsPlayer, placeHolders); }
							if (!messageSent && !handledPlayers.contains(argsPlayer.getName())) { handledPlayers.add(argsPlayer.getName()); }
							PlayerHandler.quickCraftSave(argsPlayer);
						} else if (!messageSent) { 
							if (!sender.getName().equalsIgnoreCase(argsPlayer.getName())) { ItemJoin.getCore().getLang().sendLangMessage("commands." + (remove ? "remove.targetTriedRemoval" : "get.targetTriedGive"), argsPlayer, placeHolders); }
	...
	*/
	private void handleAllItems(final CommandSender sender, final String[] args, final boolean remove) {
		...
		String[] placeHolders = ItemJoin.getCore().getLang().newString(); placeHolders[1] = (args.length >= 2 ? args[1] : sender.getName());
		if (argsPlayer == null) { ItemJoin.getCore().getLang().sendLangMessage("commands.default.noTarget", sender, placeHolders); return; }
		...
		for (ItemMap itemMap: ItemUtilities.getUtilities().getItems()) {
			if ((!remove ? (itemMap.inWorld(argsPlayer.getWorld()) && ((probable != null && itemMap.getConfigName().equals(probable.getConfigName())) || itemMap.getProbability() == -1)  && ItemUtilities.getUtilities().canOverwrite(argsPlayer, itemMap) && 
				...
				if ((remove && itemMap.hasItem(argsPlayer, true)) || ((!remove && !itemMap.hasItem(argsPlayer, false)) || (!remove && itemMap.isAlwaysGive()))) {
					if (remove || (!remove && itemMap.conditionMet(argsPlayer, "trigger-conditions", false))) {
		...
		if (itemGiven) {
			...
			if (!sender.getName().equalsIgnoreCase(argsPlayer.getName())) { placeHolders[1] = argsPlayer.getName(); ItemJoin.getCore().getLang().sendLangMessage("commands." + (remove ? "remove.removedTarget_All": "get.givenTarget_All"), sender, placeHolders); }
			...
			if (!sender.getName().equalsIgnoreCase(argsPlayer.getName())) { placeHolders[1] = sender.getName(); ItemJoin.getCore().getLang().sendLangMessage("commands." + (remove ? "remove.targetTriedRemoval_All": "get.targetFailedInventory_All"), argsPlayer, placeHolders); }
		}
		if (failedPermissions) { placeHolders[1] = argsPlayer.getName(); ItemJoin.getCore().getLang().sendLangMessage("commands.get." + (!sender.getName().equalsIgnoreCase(argsPlayer.getName()) ? "targetNoPermission_All" : "noPermission_All"), sender, placeHolders); }
	}
	    ...
	    */
		private boolean hasSyntax(final String[] args, final int page) {
			return ((args.length >= 2 && (args[1].equalsIgnoreCase(String.valueOf(page)) || (page == 2 
				 && StringUtils.isInt(args[1]) && Integer.parseInt(args[1]) != 0 && Integer.parseInt(args[1]) != 1 && ((this.equals(Execute.PERMISSIONS) && Integer.parseInt(args[1]) <= ItemData.getInfo().getPermissionPages()) 
				 || (this.equals(Execute.LIST) && Integer.parseInt(args[1]) <= ItemData.getInfo().getListPages()))) 
