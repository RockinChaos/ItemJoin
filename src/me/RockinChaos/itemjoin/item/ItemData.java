...
public class ItemData {
    ...
    * @return If OP and or Creative is defined for the Prevent actions.
    */
    ...
    * 
    * @param player - The player being checked for bypass privileges.
    * @param preventName - The prevent being checked.
    ...
    */
	public boolean triggerEnabled(final String type) {
		if (ItemJoin.getCore().getConfig("config.yml").getString("Active-Commands.triggers") != null && StringUtils.containsIgnoreCase(ItemJoin.getCore().getConfig("config.yml").getString("Active-Commands.triggers"), type)
    ...
    */
	public boolean clearEnabled(final String type) {
		if (ItemJoin.getCore().getConfig("config.yml").getString("Clear-Items." + type) != null 
		&& !ItemJoin.getCore().getConfig("config.yml").getString("Clear-Items." + type).equalsIgnoreCase("DISABLED") && !ItemJoin.getCore().getConfig("config.yml").getString("Clear-Items." + type).equalsIgnoreCase("FALSE")) {
			return true;
		}
		return false;
	}
    ...
    */
	public int getHotbarSlot() { 
		if (ItemJoin.getCore().getConfig("config.yml").getString("Settings.HeldItem-Slot") != null 
				&& !ItemJoin.getCore().getConfig("config.yml").getString("Settings.HeldItem-Slot").equalsIgnoreCase("DISABLED") 
    ...
    */
	public String getHotbarTriggers() { 
		if (ItemJoin.getCore().getConfig("config.yml").getString("Settings.HeldItem-Triggers") != null 
				&& !ItemJoin.getCore().getConfig("config.yml").getString("Settings.HeldItem-Triggers").equalsIgnoreCase("DISABLED") 
				&& !ItemJoin.getCore().getConfig("config.yml").getString("Settings.HeldItem-Triggers").equalsIgnoreCase("FALSE")) {
  ...
  /**
    ...
    */
	public void setPages() {
		SchedulerUtils.runAsync(() -> {
			final int customItems = (ItemJoin.getCore().getConfig("items.yml").getConfigurationSection("items") != null ? ItemJoin.getCore().getConfig("items.yml").getConfigurationSection("items").getKeys(false).size() : 0);
			if (customItems > 15) {
    ...
    */
  	public Map <String, List<Object>> getDatabaseData() {
  		final Map <String, List<Object>> databaseData = new HashMap < String, List<Object> >();
		for (final Table tableEnum: Table.values()) {
			...
			if (selectTable != null && !selectTable.isEmpty()) {
				for (final HashMap<String, String> sl1 : selectTable) {
					...
					final List <Object> dataSet = (databaseData.get(table) != null ? databaseData.get(table) : new ArrayList<Object>());
    ...
    */
	public void registerEvents() {
	    ItemJoin.getCore().getPlugin().getCommand("itemjoin").setExecutor(new ChatExecutor());
    ...
    */
	private void registerPrevent() {
		...
		if (!StringUtils.isRegistered(PlayerLogin.class.getSimpleName())) { ItemJoin.getCore().getPlugin().getServer().getPluginManager().registerEvents(new PlayerLogin(), ItemJoin.getCore().getPlugin()); }
		if ((!StringUtils.splitIgnoreCase(ItemJoin.getCore().getConfig("config.yml").getString("Prevent." + "Chat"), "FALSE", ",") && !StringUtils.splitIgnoreCase(ItemJoin.getCore().getConfig("config.yml").getString("Prevent." + "Chat"), "DISABLED", ","))) {
			if (!StringUtils.isRegistered(Chat.class.getSimpleName())) { ItemJoin.getCore().getPlugin().getServer().getPluginManager().registerEvents(new Chat(), ItemJoin.getCore().getPlugin()); }
		}
		...
		if ((!StringUtils.splitIgnoreCase(ItemJoin.getCore().getConfig("config.yml").getString("Prevent." + "itemMovement"), "FALSE", ",") && !StringUtils.splitIgnoreCase(ItemJoin.getCore().getConfig("config.yml").getString("Prevent." + "itemMovement"), "DISABLED", ","))) {
			if (!StringUtils.isRegistered(Clicking.class.getSimpleName())) { 
				...
				if (ServerUtils.hasSpecificUpdate("1_8") && !ItemJoin.getCore().getDependencies().protocolEnabled() && !ProtocolManager.isHandling()) { ProtocolManager.handleProtocols(); }
				else if (ServerUtils.hasSpecificUpdate("1_8") && ItemJoin.getCore().getDependencies().protocolEnabled() && !ProtocolAPI.isHandling()) { ProtocolAPI.handleProtocols(); }
			}
		}
		...
		|| (!StringUtils.splitIgnoreCase(ItemJoin.getCore().getConfig("config.yml").getString("Prevent." + "Death-Drops"), "FALSE", ",") && !StringUtils.splitIgnoreCase(ItemJoin.getCore().getConfig("config.yml").getString("Prevent." + "Death-Drops"), "DISABLED", ","))) {
			if (!StringUtils.isRegistered(Drops.class.getSimpleName())) { ItemJoin.getCore().getPlugin().getServer().getPluginManager().registerEvents(new Drops(), ItemJoin.getCore().getPlugin()); }
		}
	}
	...
	public void registerListeners(final ItemMap itemMap) {
		...
		if (itemMap.isCraftingItem() && !StringUtils.isRegistered(Crafting.class.getSimpleName())) {
			...
			SchedulerUtils.runLater(40L, () -> {
				PlayerHandler.forOnlinePlayers(player -> { this.restoreCraftItems(player, TriggerType.DEFAULT); } );
				if (ServerUtils.hasSpecificUpdate("1_8") && !ItemJoin.getCore().getDependencies().protocolEnabled() && !ProtocolManager.isHandling()) { ProtocolManager.handleProtocols(); }
				else if (ServerUtils.hasSpecificUpdate("1_8") && ItemJoin.getCore().getDependencies().protocolEnabled() && !ProtocolAPI.isHandling()) { ProtocolAPI.handleProtocols(); }
			});
  	...
  	public Runnable getCreateTables() {
		return new Runnable() {
			public void run() {
				...
				Database.getDatabase().executeStatement("CREATE TABLE IF NOT EXISTS " + ItemJoin.getCore().getData().getTablePrefix() + "return_items (`World_Name` varchar(1000), `Region_Name` varchar(1000), `Player_UUID` varchar(1000), `Inventory64` varchar(1000), `Time_Stamp` varchar(1000));");
