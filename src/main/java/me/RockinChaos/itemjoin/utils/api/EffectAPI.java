...
public class EffectAPI {
    ...
    private static void particleFirework(final Player player, final String commandParticle) {
        ...
        if (projectileParts[1] != null && !projectileParts[1].isEmpty()) {
            startColor = DyeColor.valueOf(projectileParts[1].toUpperCase()).getFireworkColor();
        }
        if (projectileParts[2] != null && !projectileParts[2].isEmpty()) {
            endColor = DyeColor.valueOf(projectileParts[2].toUpperCase()).getFireworkColor();
        }
        if (projectileParts[3] != null && !projectileParts[3].isEmpty()) {
            effectType = FireworkEffect.Type.valueOf(projectileParts[3]);
        }
        ...
        SchedulerUtils.runLater((detonationDelay * 20L), fw::detonate);
    }
}